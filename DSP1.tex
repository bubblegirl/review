...
\author{\textbf{Candidate:} Anna Liza Kretzschmar\\
 \textbf{Supervisor:} Shauna Murray \\
...
For over 15 year, \emph{Gambierdiscus} was considered to be a monotypic taxon, but since 1995 new species are being discovered with the aid of genetic analysis tools \cite{faust1995observation,holmes1998gambierdiscus,litaker2009taxonomy,chinain1999morphology,fraga2011gambierdiscus,nishimura2014morphology}. Evidence suggests that the production of CTXs and MTXs varies between species \cite{chinain2010growth,holland2013differences}, similar to the situation in other dinoflagellates, in which toxin production is highly variable between species, and generally stable within species.
...
\section{Ciguatera fish poisioning}
...
Dinoflagellates have some unique genetic features in both structure and regulation. Their genetic material extensively features repetitive genomic elements, high gene copy numbers and a large genome size ranging between 1.5 and 245 Gb (cite). These features have confounded current sequencing technologies as assembly becomes unfeasable. DNA resides in a crystallised, permanently condensed state throughout the cell cycle except during DNA replication with loops of active regions extending for access by polymerases (cite). Histones play a minor role in DNA packaging for dinoflagellates (cite).\\

To overcome these obstacles for sequencing, transcriptomics can be used (cite Murray '12 letter). This field examines the RNA of the organism, rather than the DNA. RNA is notoriously unstable, so after successful extraction it is reverse transcribed into complimentary DNA (cDNA) which can then be explored. This allows a glimpse into the actively used genes of the organism and traverses the truncated or inactive multi copies of that gene.\\

Dinoflagellates usually grow as symbionts, predators or are otherwise dependent on other organisms in a mixed system, which means that they are generally not axenically culturable. Hence if RNA is extracted from a dinoflagellate culture, it is likely to contain the transcripts of non target organisms also. This posed a major obstacle for assigning transcripts to an organism with any certainty until recently dinoflagellate specific spliced leader (dinoSL) was discovered (cite zhang and Lin '08). During the maturation of mRNA, these dinoSL are \emph{trans}-spliced to the 5' end of transcripts along with the polyadenylated 3' tail characteristic of eukaryotes. The presence of this dinoSL allows certain designation of a transcript to a dinoflagellate  with a high degree of certainty as demonstrated in case studies in a grazer-prey setting (cite Lin and Zhang '10), a natural aquatic ecosystem (cite Lin '10) and in a mixed system with \emph{Symbiodinium kawagutii} (cite Zhang '13). In the latter case study, no cDNAs from non-dinoflagellate algae present at extraction were detected (cite Zhang '13).

\section{Polyketide synthase genes in Dinoflagellates}
Dinoflagellates are prolific producers of marine biotoxins. While the toxin structure and hence mode of action varies between genera, a common feature is the basic polyketide structure. They are secondary metabolites which have undergone several steps of transformation from a linear polyketide precursor by catalytic domains of polyketide synthases (PKSs), like in fatty acid synthesis (FAS) (cite Rein and Borrone '99).\\ %Pawlowiez paper
The minimum of catalytic domains involved in chain elongation are beta-ketosynthase (KS), acyltransferase (AT) and the acyl carrier protein (ACP); for chain modification ketoreductase (KR), dehydrase (DR) and enoylreductase (ER) are required; and finally the polyketide is released from the PKS complex by thioesterase (TE). There are three major types of PKSs. Type I, which contains all the catalytic domains required, or Type II, which comprise of mono-functional units with each catalytic domain on a separate peptide which assemble into complexes and Type III which is akin to type II but smaller (insert appropriate citations). There are two functional groups within type I PKS - in modular Type I PKSs the catalytic domains are organised into modules according to each step, and that module is only used once during polyketide synthesis. In iterative type I PKSs the same set of catalytic domains are located on  one protein and used cyclically for chain elongation.\\ 
Dinoflagellate specific PKS encoding transcripts to date sport a singular catalytic domain for approximately 50 - 100 kDa proteins, reminiscent of the multi protein complex formation of type II PKS but sequence similarity is akin to type I PKS (cite Monroe and van Dolah '08, Eichholz '12). Due to this unique feature, they are termed type I-like PKS (cite Monroe '10).\\
As PKS are active in many essential pathways, such as fatty acid synthesis for the cell wall, elucidating which PKS are essential for cell function and which are specific to toxin production is of special interest. Studies focusing on PKS have been conducted for \emph{Alexandrium ostenfeldii} (cite Eichholz '12), \emph{Amphidinium} (cite Murray '12), \emph{Azadinium spinosum} (cite Meyer '15), \emph{Gambierdiscus} spp. (cite Kohli '15, Pawlowiez '14), \emph{Hererocapsa} spp. (cite Salcedo '12, Eichholz '08), \emph{Karenia brevis} (cite Monroe van Dolah '08, ryan '14) xx  \\ %ref kohli 15, google trawl dino pks
Comparative transcriptomics between toxic and non-toxic strains of dinoflagellates can help differentiate between PKS essential for cell functionality and PKS active in toxin production(cite Murray '12 letter). The continued exploration and publication of toxic dinoflagellate transcriptomes is building the framework for toxin related PKS detection. The application for monitoring blooms could revolutionise the detection of developing toxic systems and warn local populations as well as the seafood industry.


\subsection{PKS genes in \emph{Gambierdiscus} spp.}
The transcriptome of the CTX producer \emph{G. polynesiensis} was sequenced with a focus on PKS production (cite Pawlowiez '14). The study uncovered 22 transcripts with sequence similarity to type I-like PKS, however no FAS conserved domains were found. A similar observation was made in \emph{Karenia brevis} (cite), which suggests that FAS activity may be performed by PKS (cite Pawlowiez '14). Alternatively, the sequence depth and coverage may have been too shallow to detect these sequences (cite Kohli '15).

In a recent study he MTX-1 producer \emph{G. australes} was compared to \emph{G. belizeanus}, whose toxin profile did not show any MTX-1 (cite Kohli '15). Both strains produced MTX-3. This study was conducted with a focus on PKS detection and combined their transcriptomic data with the dinoflagellate transcriptomes available on MMETP. Based on the KS domain of type I-like PKS genes, they found 5 distinct clades. KS of clades C, D, and E, containing 34, 20 and 74 sequences respectively, were found in all dinoflagellate taxa analysed in this study which indicates that PKS from these clades correspond to common cellular pathways such as FAS. KS from clade A, containing 55 sequences, correlate with dinoflagellates that produce polyether ladder compounds while KS from clade B contained two sequences which were specific to \emph{G. australes} and \emph{G. belizeanus}. The authors postulate that clade A may contain PKS related to toxin production while clade B may related to MTX production (cite Kohli '15). Further in depth transcriptomic coverage of toxin producing dinoflagellates for comparison, and in particular \emph{Gambierdiscus} spp., is required to add support to this theory.

\section{Phylogeny of \emph{Gonyaulacales}}
The phylogeny of dinoflagellates is not well understood. The majority of analyses are conducted using rDNA. Murray et al investigated the evolutionary rate of rDNA using GenBank sequences for LSU and SSU, with 42 and 56 sequences respectively. They found that evolution was non-linear. Their findings support that mutational sites were linked through co-evolution, in theory to keep the integrity of the stem loop structure, but also that different sites evolve at different rates as well as multiple substitutions at the same site. Most phylogenetic programs assume compositional stationarity, which does not hold true for genes with these properties (cite Murray '05). A potential solution to the problem of differential gene evolution between taxa is to concatenated a number of genes, taking into account the different parameter settings (cite Gontcharov '04, Pupko '02).\\

- new data available for deeper informative depth of which genes are functional though transcriptomics\\
- which genera in MMETP?\\
- discuss Bachvaroff paper\\

The most recently developed clade within the dinoflagellates are the gonyaulacales (cite saldarriaga '04). This clade was also found to exhibit an exclusive character state when LSU and SSU sequences were concatenated (cite Murray '05). These factors suggest that this clade could be a good starting point for elucidating the murky evolutionary relationship of within the dinoflagellates by using a multi gene concatenated approach. If successful this could establish a framework for other clades, on the path of finding the phylogeny of dinoflagellates as a whole.
...
The isolates were separated by single cell isolation and grown as clonal cultures in F/10 medium. The cultures were maintained in F/10 medium at 26 degrees Celsius and xxx light.
...
Extracted DNA was used as a template to amplify rDNA sequences in 25$\mu$l mixture in PCR tubes. The mixture contains 0.6 $\mu$M forward and reverse primer, 0.4 $\mu$M BSA, 2 - 20 ng DNA, 12.5 $\mu$l 2xEconoTaq () and 7.5 $\mu$l MilliQ water.\\
...
The program used to analyse the sequences and their phylogenetic relationship is Geneious 8.2.1 (cite).
...
include table of potential genes -- will fill this in while you're away
