...
\title{\textbf{ \emph{Gambierdiscus} spp. -  diversity and genetics in Heron Island and the search for toxin genes responsible for ciguatoxin production, the putative causative agent for ciguatera fish poisoning.}}
...
Interest in benthic dinoflagellates was sparked in 1977, when a new species, later named \emph{Gambierdiscus toxicus}, was first suggested to produce the causative agent of ciguatera fish poisoning (CFP) \cite{yasumoto1977finding}.  
Since then, extensive research has established that species of the genus are the primary sources of ciguatoxins (CTXs) and maitotoxins (MTXs) \cite{chinain1997intraspecific,holmes1998gambierdiscus}. CTXs bioaccumulate through the food web until seafood reaches the orally accumulative toxicity level which cause CFP \cite{bagnis1979clinical,gillespie1987possible,sims1987theoretical}. %Whether MTXs play a role in CFP needs to be further investigated \cite{kohli2014feeding}. 
...
\emph{Gambierdiscus} spp. are considered a harmful algal bloom (HAB) organism, which are predicted to increase in frequency with global warming (ref).
...
The interrelationship between the species level toxicity and distribution is important information for designing a monitoring system for CFP \cite{testerICHA}.
The current state of biogeographic knowledge for most benthic dinoflagellates is poor. Species of \emph{Gambierdiscus} have been found along the coast line of many countries, but the difficulty in species identification has caused  confusion regarding the genus' biogeographic patterns \cite{marine2014}.
...
In terms of understanding difficulties in species identification, one example is \emph{G. yasumotoi}. \emph{G. yasumotoi} has been primarily reported in the Pacific, except for one report in the Mexican-Caribbean \cite{hernandez2004species}. However this singular record precedes the discovery of the other globular species \emph{G. ruetzleri}, which is considered to be endemic to the Atlantic region \cite{litaker2009taxonomy}. However a recent report of this species at the southern Kuwait coast and the Gulf of Aqaba in Jordan \cite{saburova2013new}, could indicate that this species is globally distributed also \cite{xu2014distribution}.\\
...
\emph{G. belizeanus}, \emph{G. toxicus} and \emph{G. yasumotoi} have been found in Queensland, through ..method..  \cite{murray2014molecular}. 

Kohli et al detected \emph{Gambierdiscus} spp. in Exmouth and Broome, Western Australia, via pyrosequencing \cite{kohli2014cob}.

\emph{G. toxicus}? {\large Murray 10 Benthic dinoflagellates. Algae in Australia:Phytoplankton of temperate coastal waters. Hallegraeff. bolch. CSIRO}.
Toxin profile was not established for these isolates in Australia.
...
Toxin profile analysis was conducted by Tim Harwood at the Cawthorne Institute, Germany via LC-MS.
