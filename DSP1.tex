...
\subsection{Phylogenetics og \emph{Gambierdiscus}}
...
While such large genomes cannot readily be sequenced, transcriptomics can be used for gene discovery and for examining genetic regulation (cite Murray letter '12). This method involves the extraction of mRNA from an organism, which due to its instability, is then reverse transcribed into complimentary DNA (cDNA) which can then be explored. This allows a glimpse into the actively used genes of the organism and traverses the truncated or inactive copies of that gene.

Dinoflagellates are generally not axenically culturable. Hence if RNA is extracted from a dinoflagellate culture, it is likely to contain the transcripts of bacterial organisms also. This posed a major obstacle for assigning transcripts to an organism with any certainty. Recently dinoflagellate specific spliced leader (dinoSL) sequence was discovered (cite zhang and Lin '08). This process whereby, during the maturation of mRNA, these dinoSL are \emph{trans}-spliced to the 5' end of transcripts along with the polyadenylated 3' tail characteristic of eukaryotes. The presence of this dinoSL allows certain designation of a transcript to a dinoflagellate  with a high degree of certainty as demonstrated in case studies in a grazer-prey setting (cite Lin and Zhang '10), a natural aquatic ecosystem (cite Lin '10) and in a mixed system with \emph{Symbiodinium kawagutii} (cite Zhang '13). In the latter case study, no cDNAs from non-dinoflaggelate algae present at extraction were detected (cite Zhang '13).
...
-- update following  paragraph with Orr
The most recently developed clade within the dinoflagellates are the gonyaulacales (cite saldarriaga '04). This clade was also found to exhibit an exclusive character state when LSU and SSU sequences were concatenated (cite Murray '05). These factors suggest that this clade could be a good starting point for elucidating the murky evolutionary relationship of within the dinoflagellates by using a multi gene concatenated approach. If successful this could establish a framework other clades, on the path of finding the phylogeny of dinoflagellates as a whole.
...
\subsection{Phylogeny of \emph{Gonyaulacales}}
Paramount for elucidating phylogeny is a large sample size of organisms ie. nodes on the tree. Ideally, every genus should be represented. However the limiting factor is access to transcriptomes. The main source for transcriptomes will be from the MMETP, available online at xxxx. Transcriptomes retrieved by this database will be supplemented by transcriptomes available from studies. Nucleotide sequences will be converted to AA sequences which ignores irrelevant base pair mutations (cite). important: AA, not actually concatenating but using the other one, working out if Bayesian or ML.. find which paper should go as guidelines. Gblocks? Def Bachvaroff/Genetaki papers. check out softwares
The assembly of the sequences is  a screening for contamination, alignment and concatenation process 
