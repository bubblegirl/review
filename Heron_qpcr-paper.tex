...
The 4 species of \emph{Gambierdiscus} that have been identified from Queensland and New South Wales are as follows: \emph{G. belizeanus} \citep{murray2014molecular}, \emph{G. honu} (Smith et al. 2017, in review), \emph{G. carpenteri} \citep{kohli2014high}, \emph{G. lapillus} \cite{kretzschmar2016characterization} and \emph{G. toxicus} \citep{hallegraeff2010algae}, as well as \emph{F. paulensis} (identified as \emph{F. yasumotoi}  by Murray et al. (2014) \citep{murray2014molecular}, reclassified by \cite{laza2016characterization}). Using pyrosequencing, \textit{Gambierdiscus} was identified \emph{Gambierdiscus} to the genus level in Western Australia \citep{kohli2014cob} , indicating that this is a coastline that should be examined further for CFP risk. Despite the prevalence of CFP in Australia, a species of \textit{Gambierdiscus} that consistently produces ciguatoxins has not yet been identified.
...
\section{Results}
\subsection{Evaluation of primer specificity}
The primers were specific to their target species for both \emph{G. lapillus} and \emph{G. polynesiensis} (table ~\ref{tbl:CrossreactTable}). The PCR  amplification is visualised in Fig. ~\ref{fig:lapgel}. 
\FloatBarrier 
\begin{figure} 
\includegraphics[scale=.6]{Gel_primer-spec.png} 
\caption{Gel electrophoresis visualisation of PCR using the (A) qGlapSSU2F and qGlapSSU2R primer sets; and (B)  on \emph{G. lapillus}, \emph{G. belizeanus}, \emph{G. cheloniae}, \emph{G. pacificus} and \emph{G. scabrosus} gDNA as templates.} 
\label{fig:lapgel}
\end{figure} 
\FloatBarrier
...
%explain choice for both lap and poly
