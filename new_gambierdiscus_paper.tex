...
\title{\textbf{ Morphology and phylogenetics of\emph{Gambierdiscus petramus} sp. nov. (Gonyaulacales): a new dinoflagellate associated with toxins from the Great Barrier Reef (Australia)}}
...
\caption{Sample collection site at Heron Island.} 
\label{fig:HeronMap}
\end{figure} 
\FloatBarrier 
\newpage
...
Bayesian inference, using xx, was used to estimate the posterior probability distribution with Metropolis-Coupled Markov Chain Monte Carlo (ref 53 in Nishimura 13). A random staring tree with three heated and one cold chain with a temperature set at 0.2.
...
\begin{figure} 
\includegraphics[scale=.4]{SSU_phylo_temp.jpg} 
...
\item change in cell shape and size from clonal strains \cite{bravo2014cellular}
...
\item compare toxicity to Litaker '10 MBA table
