...
\hline
 Grouper & \emph{Cephalopholis argus} (Blue spotted grouper) & Nuku Hiva, French Polynesia \cite{darius2007ciguatera}; French Polynesia \cite{bagnis1987use}; Hawaii, USA \cite{campora2008detection} & CTX +ve \cite{darius2007ciguatera,campora2008detection,bagnis1987use} & Cat BA \cite{bagnis1987use}; ELISA \cite{campora2008detection}; MBA \cite{bagnis1987use};  MQBA \cite{bagnis1987use}; N2A \cite{campora2008detection}; RBA \cite{darius2007ciguatera} \\
  &  \emph{Cephalopholis miniata} (Coral grouper) & Arafura Sea, Australia \cite{}; Republic of Fiji \cite{} & P-CTX-1 \cite{} & HPLC/MS \cite{}; MBA \cite{}; N2A \cite{} \\
  & \emph{Epinephelus coioides} (Orange-spotted grouper) & Hong Kong, China \cite{} & CTX +ve \cite{} & MBA \cite{} \\
  & \emph{Epinephelus lanceolatus} (Giant grouper) & Hong Kong, China \cite{} & CTX +ve \cite{} & MBA \cite{}  \\
  ...
  & \emph{Epinephelus mystacinus} (Misty grouper)  & St. Thomas, US Virgin Islands \cite{} & CTX +ve \cite{} & BSBA \cite{}; MGBA \cite{} \\
  & \emph{Epinephelus morio} (Red grouper) & St. Barthelemy \cite{} & CTX +ve\cite{} & MBA \cite{}; TLC \cite{} \\
  &  \emph{Epinephelus} sp. & Baja California, Mexico \cite{} & CTX +ve \cite{} & MBA \cite{} \\
  & \emph{Mycteroperca bonaci}  (Black grouper) & Key Largo, USA  \cite{} & C-CTX-1 \cite{}; C-CTX-2 \cite{} & LC-MS/MS \cite{}; N2A \cite{} \\
  & \emph{Mycteroperca prionura} (Sawtail grouper) & Baja California, Mexico \cite{} & CTX-1 \cite{} & HPLC/MS \cite{}; MBA \cite{} \\
  &  \emph{Mycteroperca} sp. & Baja California, Mexico \cite{} & CTX +ve \cite{} & MBA \cite{} \\
  ...
  & \emph{Plectropomus aerolatus} (Squaretail coral grouper) & Hong Kong, China \cite{} & CTX +ve \cite{} & MBA \cite{} \\
  &  \emph{Plectropomus laevis} (Blacksaddles coral grouper) & Hong Kong, China \cite{} & CTX +ve \cite{} & MBA \cite{}\\
  & \emph{Plectropomus leopardus}  (Leopard coral grouper) & Tahiti, French Polynesia \cite{}; Tubuai, French Polynesia \cite{darius2007ciguatera}; French Polynesia \cite{bagnis1987use}; Hong Kong, China \cite{} & CTX +ve \cite{darius2007ciguatera,bagnis1987use} & Cat BA \cite{bagnis1987use}; MBA \cite{bagnis1987use}; MQBA \cite{bagnis1987use}; RBA \cite{darius2007ciguatera} \\
  & \emph{Plectropomus} spp. (Coral trout) & Great Barrier Reef, Australia \cite{} & CTX-1 \cite{}; CTX-2 \cite{}; CTX-3 \cite{} & HPLC/MS \cite{}; MBA \cite{} \\
  &  \emph{Serranidae} & French West Indies \cite{} & CTX-1 \cite{}; CTX-2 and isomers \cite{}; CTX congeners and other \cite{} & MBA \cite{} \\ % CHECK OUT THIS REF TO GET A SPECVUES ... da fuck really? MBA gives that much resolution?
  & \emph{Variola albimarginata}  & Hong Kong, China \cite{} & CTX +ve \cite{} & MBA \cite{} \\
  \hline
 Grunt & \emph{Pomadasys maculatus} (Blotched javelin) & Platypus Bay, Australia \cite{} & CTX-1 \cite{}; CTX-2 \cite{}; CTX-3 \cite{} & HPLC/MS \cite{}; MBA \cite{} \\
  &  \emph{} &  &  & \\
  & \emph{}  &  &  & \\
  & \emph{} &  &  & \\
  &  \emph{} &  &  & \\
